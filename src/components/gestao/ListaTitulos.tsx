...
import { Botao, Campo, Cartao, CartaoCorpo, Etiqueta, Rotulo, Selecao, Vazio } from "@/components/ui";
...
export type FiltrosTitulos = { status?: string; de?: string; ate?: string; busca?: string };
...
export function IndicadoresTitulos({ titulos }: { titulos: TituloComSaldo[] }) {
  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
      <Indicador Icone={Wallet} rotulo="Em aberto" valor={emAberto.reduce((s, t) => s + Number(t.saldo), 0)} />
      <Indicador Icone={AlertTriangle} rotulo="Vencido" valor={vencidos.reduce((s, t) => s + Number(t.saldo), 0)} tom="vermelho" />
      <Indicador Icone={Clock} rotulo="Vence em 30 dias" valor={proximos.reduce((s, t) => s + Number(t.saldo), 0)} />
...
export function FiltrosBarra({ filtros }: { filtros: FiltrosTitulos }) {
  return (
      ...
      <CartaoCorpo>
        <form className="grid gap-4 sm:grid-cols-5 sm:items-end">
          ...
          <div>
            ...
            <Campo id="busca" name="busca" placeholder="Fornecedor, membro, documento" defaultValue={filtros.busca ?? ""} />
          </div>
      ...
      <table className="w-full min-w-[46rem] text-sm">
        <thead>
          <tr className="border-b border-borda bg-azul-50/60 text-left">
            <th className="px-5 py-3 font-semibold text-texto">Vencimento</th>
            ...
            <th className="px-4 py-3 text-right font-semibold text-texto">Valor</th>
            <th className="px-4 py-3 text-right font-semibold text-texto">Saldo</th>
            <th className="px-4 py-3 font-semibold text-texto">Situação</th>
          </tr>
        </thead>
